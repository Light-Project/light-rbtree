...
#define rb_post_first_entry(ptr, type, member) \
    rb_entry(rb_post_first(ptr), type, member)

#define rb_post_next_entry(pos, member) \
    rb_entry(rb_post_next(&(pos)->member), typeof(*(pos)), member)

#define rb_post_for_each_entry(pos, head, member)               \
    for (pos = rb_post_first_entry(head, typeof(*pos), member); \
         &pos->member != NULL;                                  \
         pos = rb_post_next_entry(pos, member))
